# template.py - IBM Systems (ISDL) Interview Preparation Portal HTML Renderer

def render_ibm_page(title, active_page, content_html, prev_link=None, prev_title=None, next_link=None, next_title=None):
    nav_items = [
        ("index.html", "Overview & Hub", "Portal"),
        ("00_START_HERE.html", "00: Company & Role Intel", "Strategy"),
        ("01_Online_Test.html", "01: Signature OA Problems", "Algorithms"),
        ("02_Technical_Rounds.html", "02: Systems Programming & OS", "Core Systems"),
        ("03_Domain_Deep_Dive.html", "03: POWER10, z16 & OpenBMC", "Architecture"),
        ("04_Candidate_Resume_Grilling.html", "04: Resume Defense & Traps", "Defense"),
        ("05_System_Design_or_HIL.html", "05: Firmware Telemetry LLD", "System Design"),
        ("06_Managerial_and_HR.html", "06: Life at IBM & STAR Stories", "Behavioral"),
        ("07_Quick_Reference.html", "07: Caveman Systems CheatSheet", "CheatSheet"),
        ("README.html", "3-Day Study Roadmap", "Roadmap"),
    ]
...
:root {{
  ...
  --accent-primary-hover: #0043ce;
  --accent-light: rgba(15, 98, 254, 0.1);
...
[data-theme="dark"] {{
  ...
  --bg-card: #151d30;
  --bg-sidebar: #0b1120;
  --bg-code-header: #1c273e;
  --bg-inline-code: #1c273e;
  ...
  --accent-primary-hover: #0f62fe;
  --accent-light: rgba(69, 137, 255, 0.12);
  
  --tag-bg: rgba(15, 98, 254, 0.18);
  --tag-border: #4589ff;
  --tag-text: #a6c8ff;
...
#reading-progress {{
  ...
  left: 0;
  height: 3px;
  background: linear-gradient(90deg, #0f62fe, #4589ff, #0043ce);
  width: 0%;
  z-index: 2000;
...
<body>
    ...
    <aside class="sidebar" id="sidebar">
      <div class="sidebar-brand">
        <a href="index.html" class="brand-left">
          <div class="brand-icon">IBM</div>
          <div class="brand-text">
            <h2>IBM Systems</h2>
            <span>ISDL Software Engineer</span>
          </div>
        </a>
        <button class="drawer-close-btn" id="drawer-close-btn" aria-label="Close Menu">✕</button>
      </div>

      <div class="nav-section-title">Preparation Modules</div>
      <ul class="sidebar-nav">
        {sidebar_links}
      </ul>

      <div class="sidebar-footer">
        <a href="../index.html" class="nav-link" style="background:var(--accent-light); color:var(--accent-primary);">
    ...
    <main class="main-wrapper">
      <header class="top-nav">
        ...
        <div style="display:flex; align-items:center; gap:8px;">
          <span class="badge-ibm">₹18.75 LPA · ISDL Core</span>
        </div>
      </header>

      <div style="max-width:960px; margin:0 auto; padding:28px 20px; width:100%;">
  ...
  <nav class="bottom-nav-bar">
    <a href="index.html" class="b-nav-item{' active' if active_page == 'index.html' else ''}">
    ...
    <a href="07_Quick_Reference.html" class="b-nav-item{' active' if active_page == '07_Quick_Reference.html' else ''}">
      <span class="b-nav-icon">⚡</span>
    ...
    <button class="b-nav-item" id="b-nav-top">
      <span class="b-nav-icon">↑</span>
      <span>Top</span>
    </button>
  </nav>
  ...
  <script>
    ...
    const bNavThemeIcon = document.getElementById('b-nav-theme-icon');
    
    function applyTheme(theme) {{
      document.documentElement.setAttribute('data-theme', theme);
      localStorage.setItem('ibm_prep_theme', theme);
      if (theme === 'dark') {{
        if (themeIcon) themeIcon.textContent = '☀️';
        if (themeText) themeText.textContent = 'Light Mode';
        if (bNavThemeIcon) bNavThemeIcon.textContent = '☀️';
      }} else {{
        if (themeIcon) themeIcon.textContent = '🌙';
        if (themeText) themeText.textContent = 'Dark Mode';
        if (bNavThemeIcon) bNavThemeIcon.textContent = '🌙';
      }}
    }}

    const savedTheme = localStorage.getItem('ibm_prep_theme') || 'dark';
      ...
      copyBtn.addEventListener('click', () => {{
        ...
        if (code) {{
          navigator.clipboard.writeText(code.textContent).then(() => {{
            ...
            copyBtn.style.background = '#0f62fe';
